...
io.on('connection', (socket) => {
    ...
    socket.on('signup', (data) => {
        if (userDB[data.user]) {
            socket.emit('authMsg', { success: false, msg: "Username taken!" });
        } else {
            // Create Account
            userDB[data.user] = { 
                password: data.pass, 
            ...
            saveDB();
            socket.emit('authMsg', { success: true, msg: "Account created! Please Log In." });
        }
    });

    socket.on('login', (data) => {
        const account = userDB[data.user];
        if (!account || account.password !== data.pass) {
            socket.emit('authMsg', { success: false, msg: "Invalid User or Password." });
        } else {
            joinGame(socket, data.user, account, data.color);
        }
    });

    socket.on('guest', (data) => {
        joinGame(socket, "Guest-" + Math.floor(Math.random()*1000), { money: 0, hat: 'none', email: '' }, data.color);
    });
    ...
    function joinGame(socket, username, stats, color) {
        ...
        let isOwner = (stats.email === 'raidenrmarks12@gmail.com');

        players[socket.id] = {
            id: socket.id,
            x: spawn.x, y: spawn.y, w: 40, h: 40,
            vx: 0, vy: 0,
            hp: isOwner ? 300 : 100,       // Owner gets 300 HP
            maxHp: isOwner ? 300 : 100,
            color: isOwner ? '#FFD700' : (color || '#4488FF'), // Owner gets Gold Color
            username: username,
            weapon: isOwner ? WEAPONS.rpg : WEAPONS.pistol,    // Owner gets RPG
            money: stats.money,
            hat: stats.hat,
