...
const KITS = {
    ...
    ninja: { id: 'ninja', name: 'Ninja', hp: 90, speed: 1.3, regen: 0.05, weapon: { damage: 25, speed: 30, cooldown: 15, ammo: 10, reload: 60, spread: 0.02, range: 50, w: 0, h: 0 } }, 
...
if(fs.existsSync(DB_FILE)) {
    try { userDB = JSON.parse(fs.readFileSync(DB_FILE)); } 
    catch(e) { console.log("DB Error, resetting"); userDB = {}; }
}
...
class Lobby {
    ...
    resetGame(mode, mapIdx) {
        ...
        if(mode === 'ZOMBIES') { this.gameState.wave = 1; this.spawnZombies(5); }
    ...
    assignTeam(p, index) {
        ...
        } else { p.team = 'FFA'; p.color = (p.isOwner && p.godMode) ? '#FFD700' : '#4488FF'; }
    }
    ...
    getSpawn(team) {
        ...
        do {
            if(this.gameState.mode === 'ZOMBIES' && team === 'ZOMBIE') {
                if(Math.random()>0.5) { x=(Math.random()>0.5?50:1950); y=Math.random()*1500; } else { x=Math.random()*2000; y=(Math.random()>0.5?50:1450); }
            }
    ...
    spawnZombies(count) {
        for(let i=0; i<count; i++) {
            let id = 'z_'+Math.random(); let s = this.getSpawn('ZOMBIE'); let hp = 50 + (this.gameState.wave * 10);
            this.players[id] = { id:id, username:`Zombie`, type:'zombie', team:'ZOMBIE', x:s.x, y:s.y, vx:0, vy:0, w:40, h:40, angle:0, hp:hp, maxHp:hp, speed:0.5 + (this.gameState.wave * 0.05), color:'#55aa55', grapple:{active:false}, hat:'none' };
        }
    }

    respawn(p) {
        if(this.gameState.mode === 'ZOMBIES' && p.lives <= 0) { p.dead = true; p.x = -1000; return; }
        p.dead = false; let s = this.getSpawn(p.team); p.x=s.x; p.y=s.y;
...
io.on('connection', (socket) => {
    ...
    const syncData = (p) => {
        ...
        if (userDB[p.username]) {
            ...
            userDB[p.username].kills = p.kills; // Save Kills
    ...
    socket.on('guest', (d) => { finishAuth(socket, d, { money:0, xp:0, kills:0, items:[] }, false); });
    socket.on('signup', (d) => {
        if(userDB[d.user]) return socket.emit('authMsg',{s:false,m:"Taken"});
        userDB[d.user] = { pass:d.pass, email:d.email, money:0, xp:0, kills:0, items:[], banned: false, isStaff: false, clan: "" };
    ...
    function finishAuth(s, d, acc, isLogin) {
        ...
        currentUser = {
            ...
            money: acc.money, xp: acc.xp, 
            kills: acc.kills || 0, // Load Kills
            level: getLevel(acc.xp), 
            items: acc.items || [], 
            equippedHat: acc.equippedHat || 'none',
            clan: acc.clan || "", 
            isOwner: isOwner, isStaff: isStaff, godMode: isOwner, isGuest: !isLogin
        };
        s.emit('authSuccess', { isOwner, isStaff, user: currentUser.username, pass: currentUser.pass, money: currentUser.money, items: currentUser.items, equippedHat: currentUser.equippedHat, clan: currentUser.clan });
        sendLobbyList(s);
    }
    ...
    socket.on('requestLeaderboard', () => {
        // Sort users by kills
        let sorted = Object.entries(userDB)
            .map(([name, data]) => ({ name: name, kills: data.kills || 0, clan: data.clan || "" }))
    ...
    socket.on('clanAction', (data) => {
        ...
        if(data.type === 'join') {
            let tag = data.tag.substring(0, 6).toUpperCase();
            currentUser.clan = tag;
            syncData(currentUser);
            socket.emit('authSuccess', { ...currentUser }); // Refresh client
            socket.emit('chatMsg', {user:'[SYS]', text:`Joined Clan [${tag}]`, color:'lime'});
        } else if (data.type === 'leave') {
            currentUser.clan = "";
            syncData(currentUser);
            socket.emit('authSuccess', { ...currentUser });
            socket.emit('chatMsg', {user:'[SYS]', text:`Left Clan`, color:'orange'});
        }
    });

    // ... (Keep existing chat logic, buy, equip, createLobby, joinLobby, input) ...
    // IMPORTANT: In chat logic, remove the old /clan command handler to avoid conflict, or keep it as alias.
    socket.on('chat', (m) => {
        ...
        if(m.startsWith('/')) {
            let args = m.split(' '); let cmd = args[0].toLowerCase(); let arg1 = args[1];
            
            // Only allow specific commands for non-staff
            if(['/help'].includes(cmd)) { /* Allow */ }
            else if(!hasAuth && !p.isStaff && !p.isOwner) { return socket.emit('chatMsg', {user:'[ERR]', text:'Unknown cmd', color:'red'}); }

            if(cmd === '/noclip' && p.isStaff) { p.noclip = !p.noclip; socket.emit('chatMsg', {user:'[SYS]', text:`Noclip: ${p.noclip}`, color:'pink'}); return; }
            // ... (Add back other admin commands from previous step: ban, kick, map, mode, etc) ...
            if(cmd === '/map' && hasAuth) { 
                let i=0, n=arg1?.toLowerCase(); 
                if(n==='maze')i=1; else if(n==='towers')i=2; else if(n==='open')i=3; 
    ...
    // ... (Standard Lobby Logic: requestLobbies, createLobby, joinLobby, input, etc.) ...
    socket.on('requestLobbies', () => sendLobbyList(socket));
    function sendLobbyList(s) {
        let list = Object.values(lobbies).filter(l => l.isPublic).map(l => ({ id: l.id, name: l.name, mode: l.gameState.mode, map: MAPS[l.gameState.mapIndex].name, count: Object.values(l.players).filter(p => p.type !== 'zombie').length }));
        s.emit('lobbyList', list);
    }
    socket.on('createLobby', (settings) => { if(!currentUser) return; let lobbyId = Math.random().toString(36).substring(2, 6).toUpperCase(); lobbies[lobbyId] = new Lobby(lobbyId, socket.id, settings); joinLobby(socket, lobbyId); io.emit('lobbyUpdate'); });
    socket.on('joinLobby', (lobbyId) => { if(!currentUser) return; lobbyId = lobbyId.toUpperCase(); if(lobbies[lobbyId]) joinLobby(socket, lobbyId); else socket.emit('authMsg', {s:false, m:"Lobby Not Found"}); });
    ...
    function joinLobby(s, lobbyId) {
        ...
        let p = {
            ...
            clan: currentUser.clan || "", noclip: false, kills: currentUser.kills // Pass kills to game object
        };
    ...
    socket.on('input', (d) => {
        ...
        if(d.grapple && !p.grapple.active) {
            if(p.kit.id === 'engineer') {
                if(!p.lastSentryTime || Date.now() - p.lastSentryTime > 5000) { 
                    lobby.sentries = lobby.sentries.filter(s => s.owner !== p.id);
                    lobby.sentries.push({ x: p.x, y: p.y, owner: p.id, team: p.team, angle: 0, hp: 80, maxHp: 80, cooldown: 0, range: 400 });
                    p.lastSentryTime = Date.now(); socket.emit('chatMsg', {user:'[SYS]', text:'Sentry Placed!', color:'lime'});
                }
            } else { p.grapple.active=true; p.grapple.x=d.gx; p.grapple.y=d.gy; }
...
function handleDeath(lobby, victim, killer) {
    if(killer && killer.type === 'player') { 
        ...
        if(userDB[killer.username]) { 
            ...
            userDB[killer.username].kills = (userDB[killer.username].kills || 0) + 1;
            saveDB(); 
        } 
    }
