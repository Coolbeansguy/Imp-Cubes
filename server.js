...
io.on('connection', (socket) => {
    ...
    socket.on('staffAction', (data) => {
        ...
        if(data.type === 'spawnDummy') {
            ...
            players[id] = {
                ...
                grapple:{active:false}, team:'FFA', score:0, level:0, hat:'none'
            };
        }
        if(data.type === 'clearDummies') {
            for(let id in players) if(players[id].isDummy) delete players[id];
        }
        if(data.type === 'kickAll') io.emit('chatMsg', { user: '[SYSTEM]', text: 'Server Restarting...', color: 'red' });
    });
    ...
    socket.on('buy', (item) => {
        ...
        if(p.money >= cost && !p.items.includes(item)) {
            p.money-=cost; p.items.push(item); p.hat=item;
            userDB[p.username].money=p.money; userDB[p.username].items=p.items;
            saveDB();
        }
    });
...
function auth(s, d, isLogin) {
    ...
    players[s.id] = {
        ...
        selectedKit: baseKit, kit: startingKit,
        ...
        shootTimer:0, dashTimer:0, grapple:{active:false}, score:0
    };
...
setInterval(() => {
    if(gameState.phase === 'GAME') {
        ...
        if(gameState.mode === 'KOTH' && gameState.hill) {
            ...
            for(let id in players) {
                let p = players[id];
                if(p.x>gameState.hill.x && p.x<gameState.hill.x+200 && p.y>gameState.hill.y && p.y<gameState.hill.y+200) {
                    if(p.team === 'RED') r++; else if(p.team === 'BLUE') b++;
                }
            }
        ...
        gameState.timer--;
        if(gameState.timer <= 0) { gameState.phase = 'VOTE'; gameState.timer = VOTE_TIME; gameState.votes = {map:{}, mode:{}}; }
    }
        ...
        if(gameState.mode === 'CTF' && !p.isDummy) {
            gameState.flags.forEach(f => {
                ...
                if(f.carrier===p.id && Math.hypot(p.x-f.base.x, p.y-f.base.y)<50) {
                    gameState.scores[p.team.toLowerCase()]++; p.score+=100;
                    f.carrier=null; f.x=f.base.x; f.y=f.base.y;
                }
            });
        }
    }
    ...
    for(let i=bullets.length-1; i>=0; i--) {
        ...
        if(!hit) {
            for(let id in players) {
                let p = players[id];
                if(b.owner!==id && (gameState.mode==='FFA' || p.team!==shooter.team) && b.x>p.x && b.x<p.x+p.w && b.y>p.y && b.y<p.y+p.h) {
                    p.hp-=b.dmg; hit=true;
                    if(p.hp<=0) {
                        if(!shooter.isDummy) { 
                            shooter.score+=10; shooter.money+=25; shooter.xp+=50; shooter.level=getLevel(shooter.xp);
    ...
    if(gameState.mode === 'CTF') {
        gameState.flags.forEach(f => { if(f.carrier && players[f.carrier]) { f.x=players[f.carrier].x; f.y=players[f.carrier].y; } });
    }
